...
import {
  ...
  sprayLockPreflightSchema,
  type SprayLockTelemetry,
} from '@/lib/spraylock';
...
function configuredDevicePublicKey(deviceId: string): string | null {
  const raw = process.env.SPRAYLOCK_DEVICE_PUBLIC_KEYS_JSON;
  if (!raw) return null;
  try {
...
function verifyDeviceAttestation(telemetry: SprayLockTelemetry): AttestationStatus {
  ...
  try {
    return verify(
      null,
      Buffer.from(deviceSignaturePayload(telemetry)),
...
export async function POST(request: NextRequest) {
  ...
  const parsed = sprayLockPreflightSchema.safeParse(payload);

  if (!parsed.success) {
    return NextResponse.json(
      { error: 'Invalid SprayLock preflight payload', issues: parsed.error.flatten() },
      { status: 400, headers: { 'Cache-Control': 'private, no-store' } },
    );
  }

  const verifiedAt = new Date().toISOString();
  const evaluation = evaluateSprayLockPreflight(parsed.data, new Date(verifiedAt));
  ...
  const canonicalRecord = canonicalizeSprayLockPayload(record);
  const signingKey = process.env.SPRAYLOCK_RECORD_SIGNING_KEY;
  ...
  return NextResponse.json(
    {
      ...record,
      evidenceHash,
      recordSealMode: signingKey ? 'server-hmac-sha256' : 'demo-digest-only',
    },
